class S extends Piece {
  ...
  public S(int x, int y, boolean active) {
    super(x, y, active);
    this.portions = new Portion[4];
    portions[0]= new Portion(this.x, this.y+30);
    portions[1]= new Portion(this.x+30, this.y);
    portions[2]= new Portion(this.x+30, this.y+30);
    portions[3]= new Portion(this.x+60, this.y);
  }
  ...
  public void rotation() {
    if (active) {
      if (this.pos==0) {
        this.portions[0].setPosition(portions[0].x+30, this.portions[0].y-60);
        this.portions[1].setPosition(portions[1].x+30, this.portions[1].y);
        this.portions[2].setPosition(portions[2].x, this.portions[2].y-30);
        this.portions[3].setPosition(portions[3].x, this.portions[3].y+30);
        pos=1;
      } else {
        this.portions[0].setPosition(portions[0].x-30, this.portions[0].y+60);
        this.portions[1].setPosition(portions[1].x-30, this.portions[1].y);
